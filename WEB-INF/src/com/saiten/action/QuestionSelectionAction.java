...
public class QuestionSelectionAction extends ActionSupport implements
	...
	public String fetchDbInstanceInfo() {
		...
		try {
			...
			throw new SaitenRuntimeException(
					ErrorCode.QUESTION_SELECTION_ACTION_EXCEPTION, e);
		}
	...
	public String onLoad() {
		try {
			...
			questionMap = questionSelectionService
					.findQuestionsByMenuIdAndScorerId(selectedMenuId, scorerId,
							roleId);
	...
	public String fetchMarkValues() {
		try {
			...
			for (Map.Entry<Integer, Short> entry : markValuesMap.entrySet()) {
				selectedQuestionMarkValueMap.put(entry.getValue(), entry
						.getValue().toString());
			}

			selectedQuestionMarkValueMap.put(new Short(WebAppConst.MINUS_ONE),
					getText(WebAppConst.OTHERS));
	...
	private void buildQuestionInfo() {
		questionInfo = (QuestionInfo) ContextLoader
	 ...
	 */
	public void setSelectedQuestionMarkValueMap(
			Map<Short, String> selectedQuestionMarkValueMap) {
