...
public class SessionInvalidateInterceptor extends AbstractInterceptor {
	...
	private static Logger log = Logger
			.getLogger(SessionInvalidateInterceptor.class);
	...
	public String intercept(ActionInvocation invocation) throws Exception {
		...
		try {
			...
			if (mstScorerInfoObj != null) {
				...
				if (tranScorerSessionInfo != null) {
					...
					if (questionSequence != null) {
						...
						if (!StringUtils.isBlank(lockBy)
							...
							UnlockAnswerUtil.unlockAnswer(questionSequence,
									lockBy, connectionString, answerSeq);
						}
					}
				...
				// update user logging information in
				// tran_scorer_access_log_info.
				 ...
				 * .getAllSessions(userQuestionInfo, false); for
				 * (SessionInformation userSession : userSessions) {
				 * UserQuestionInfo userQuestionInfoObj = new
				 * UserQuestionInfo(); userQuestionInfoObj = (UserQuestionInfo)
				 * userSession .getPrincipal(); QuestionInfo questionInfo =
				 * userQuestionInfoObj .getQuestionInfo(); if (questionInfo !=
				 * null) {
				 * 
				 ...
				 * 
				 * String answerFormNum = userQuestionInfoObj
				 * .getAnswerFormNum(); String subjectCode =
				 * questionInfo.getSubjectCode(); if (answerFormNum != null &&
				 * subjectCode != null) { SaitenUtil.updateSpecialScoringMap(
				...
				/*
				 * sessionRegistry.registerNewSession(sessionId,
				 * userQuestionInfo);
				 */
