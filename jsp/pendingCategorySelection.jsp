...
<tiles:insertDefinition name="baseLayout">
	<tiles:putAttribute name="head-title"> 
			<s:if test="#session.questionInfo.menuId == @com.saiten.util.WebAppConst@PENDING_MENU_ID">
				<title><s:text name="pending.pendingCategorySelection.title" /></title>
			</s:if>
	</tiles:putAttribute>
		...
		<table class="table_width_height">
		 <tr>
		  ...
		  <td class="table-center-menu-td">
			<div>
			  <div class="box">
			  	<s:if test="%{pendingCategoryMap.isEmpty()}">
			  		<s:actionerror/>
			  	</s:if><s:else>
			  	<h1 align="left"><s:text name="label.pendingCategorySelection.SelectCategory" />&nbsp;<s:text name="label.scoring.openingbracket" /><s:property value="subjectShortName"/><s:text name="label.scoring.closingbracket" /></h1>
